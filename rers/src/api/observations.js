...
export function parseObservation(c) {
  ...
  return {
    ...
    interactionId: view.interactionId ?? c.payload?.interactionId,
    subject:       view.subject       ?? c.payload?.subject,
    recordedAt:    view.recordedAt    ?? c.payload?.recordedAt ?? c.createdAt,
    processed:     view.processed === true || c.payload?.processed === true,
    components,
    payload:       c.payload ?? {},
  };
}
