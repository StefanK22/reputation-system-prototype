...
class CantonNodeService {
  ...
  async start() {
    if (this.seedData) seed({ publish: (t, p) => this.publish(t, p) });
    ...
    this.server = http.createServer(async (req, res) => {
      try {
        ...
        if (method === 'GET'  && pathname === '/v2/events') {
          ...
          return sendJson(res, 200, { events: this.events.filter((e) => e.offset > from) });
        }
        ...
        if (method === 'POST' && pathname === '/v2/state/active-contracts') {
          ...
          return sendJson(res, 200,
            this.events
              .filter((e) => e.offset <= atOffset && (!filters.length || filters.includes(e.templateId)))
        ...
        if (method === 'POST' && pathname === '/v2/commands/submit-and-wait-for-transaction') {
          ...
          for (const cmd of commands) {
            if (!cmd?.CreateCommand) continue;
            const event = this.publish(cmd.CreateCommand.templateId, cmd.CreateCommand.createArguments || {});
            created.push({ CreatedEvent: toCreatedEvent(event) });
          }
          if (!created.length) return sendJson(res, 400, { error: 'No CreateCommand found' });
